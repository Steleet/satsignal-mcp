...
**Honest status as of `satsignal-mcp` 0.4.1 (published 2026-05-21):** PyPI's JSON API at `https://pypi.org/pypi/satsignal-mcp/0.4.1/json` reports `provenance: null` for both the sdist and the wheel. The workflow is configured to emit attestations but the artifact is not yet visible on PyPI; root-cause is open.
