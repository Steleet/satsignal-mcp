...
- **`anchor_disclosable`** — anchor a `.txt` (text-tree-v1) or `.json` (json-ast-v1) payload as a SEALED deep-content envelope: per-leaf commitments under one Merkle root, so any subset can be disclosed later and still verify against the same on-chain commitment. Hashing + envelope build are LOCAL; only the root is broadcast and the master salt is never returned. `storage` selects `mirror` (default — the salt is persisted in the source `.mbnt` so redaction works later from the bundle alone) or `blind` (salt stays local, never uploaded). Optional `reveal`/`reveal_names` emit a one-shot disclosure in the same call (requires `mirror`). Honors `dry_run` (build + preview, no broadcast, no quota).
- **`create_disclosure`** — LOCAL-ONLY redaction from an already-anchored source `.mbnt`: reveal a chosen subset (0-based indices, or json-ast RFC-6901 pointers / text-tree slash paths), seal the rest, self-verify, and write a `.disclosure.mbnt`. No network, no quota — redact the same source for many audiences. `list_only` enumerates the selectors first.
- **`verify_disclosure`** — confirm a disclosure cryptographically binds to the committed Merkle root (revealed-leaf recompute + proof-path walk + carrier/profile/algo binding, plus an optional redacted-view sha256 check). `verified=false` is a *successful* result, not a tool error (mirrors `verify_file_against_bundle`). On-chain existence is surfaced via `linked_txid` + `root` for explorer confirmation — `lookup_hash` does not index sealed/deep-content anchors (the same reason `chain_confirm_bundle` returns `no_file_sha` for them).
- **Honesty boundary (unchanged):** a disclosure proves the revealed fields are authentic to the sealed commitment and the sealed fields are provably present-but-hidden (tamper-evidence + timing) — NOT authorship, NOT that the content pre-existed the anchor, NOT that the content is true.
- **node>=18 host prerequisite (the one real cost).** These three tools shell out to a VENDORED snapshot of the JS disclosure-builder (monorepo decision 0023 — one source of truth for the leaf / RFC-8785 JCS / duplicate-last Merkle / HKDF per-leaf-salt crypto; a Python re-port would risk a one-byte divergence from the on-chain anchor). The snapshot ships inside the wheel/sdist under `satsignal_mcp/vendor/disclosure_builder/` with a `SOURCE_COMMIT` provenance stamp; re-sync via `scripts/sync_vendored_builder.sh`. Node is found on PATH or via `SATSIGNAL_NODE`; its absence is the fail-closed `node_unavailable` error, never a silent skip. The other seven tools remain pure-Python with no node dependency. The API key is passed to the Node subprocess for the `anchor` op only — redaction/verify/list never receive it.
